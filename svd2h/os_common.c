...
#define MIN_ARENA_SIZE         (256*1024)
#define ARENA_GUARD_VALUE      0x59a4b349
...
//-----------------------------------------------------------------------------
OsArena *os_arena_new(int size)
{
  if (size < MIN_ARENA_SIZE)
    size = MIN_ARENA_SIZE;

  OsArena *arena = os_alloc(sizeof(OsArena) + size);

  arena->incr  = size;
  arena->size  = size;
  arena->data  = (u8 *)arena + sizeof(OsArena);
  arena->next  = NULL;
  arena->guard = ARENA_GUARD_VALUE;

  return arena;
}

//-----------------------------------------------------------------------------
void *os_arena_alloc(OsArena *arena, int size)
{
  os_assert(arena);
  os_assert(size >= 0);
  ...
  while (1)
  {
    os_assert(arena->guard == ARENA_GUARD_VALUE);

    if (arena->size >= size)
      break;

    if (arena->next == NULL)
      arena->next = os_arena_new(arena->incr);
...
//-----------------------------------------------------------------------------
void os_arena_free(OsArena *arena)
{
  while (arena)
  {
    OsArena *ar = arena;
...
{
  for (const OsOption *opt = options; opt->value; opt++)
  {
